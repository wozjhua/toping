...
项目的核心目标是极低延迟安卓投屏和 PC 映射控制。主画面链路采用 JPEG 单帧直传，用更高带宽换取更短端到端链路，避开传统视频流中的编码器队列、GOP、解码重排和播放同步缓冲。为了降低带宽压力，项目支持 ROI 区域策略：中心区域保持高质量，外围区域降低质量或缩放编码。
...
极低延迟模式依赖高性能 Android 设备、稳定散热、USB 3.0 及以上链路，以及未被系统温控明显降频的运行状态。推荐使用 Snapdragon 8 Elite 级别或同等级旗舰 SoC，并配合良好散热；如果设备已经因温控降频，高 FPS JPEG 编码链路的延迟和稳定性都会受到影响。

USB 2.0 有效带宽较低，在 1080P、高 FPS、较高 JPEG 质量下容易成为瓶颈。即使项目通过 ROI 策略降低外围区域质量、减少传输量，仍建议优先使用 USB 3.0 及以上的数据线和接口来复现最佳低延迟效果。
